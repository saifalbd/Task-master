<template>
    <app-layout :busy="busy">
        <div>
            <page-title-box title="Event Categories">
                <create-button title="Add Event Category" @click="showCreate = true"></create-button>
            </page-title-box>
        </div>
...
import Create from "./CreateEventCategory.vue";
import Edit from "./EditEventCategory.vue";
import { ref} from "vue";
...
export default {
    ...
    setup() {
        ...
        const fetchItems = async (page) => {
            busy.value = true;
            const url = route("eventCategory.index", {
        ...
        const remove = async (item, index) => {
            const is = await confirm(item, "Event Category");
            if (!is) return null;
            try {
                const url = route("eventCategory.destroy", { category: item.id });
